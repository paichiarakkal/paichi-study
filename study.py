...
if not st.session_state.auth:
    ...
    # --- PAGES ---
        ...
        # --- കലണ്ടർ ലോജിക് ഇവിടെ തുടങ്ങുന്നു ---
        ...
        # തീയതി അനുസരിച്ച് ക്രെഡിറ്റും ഡെബിറ്റും ഗ്രൂപ്പ് ചെയ്യുന്നു
        daily = df_cal.groupby(df_cal['Date'].dt.date)[['Credit', 'Debit']].sum().reset_index()
        
        calendar_events = []
        for _, row in daily.iterrows():
            if pd.notnull(row['Date']):
                # ക്രെഡിറ്റ് ഉണ്ടെങ്കിൽ പച്ച നിറത്തിൽ കാണിക്കുക
                if row['Credit'] > 0:
                    calendar_events.append({
                        "title": f"⬆️ ₹{row['Credit']:,.0f}", 
                        "start": str(row['Date']), 
                        "color": "#28a745"
                    })
                # ഡെബിറ്റ് ഉണ്ടെങ്കിൽ ചുവപ്പ് നിറത്തിൽ കാണിക്കുക
                if row['Debit'] > 0:
                    calendar_events.append({
                        "title": f"⬇️ ₹{row['Debit']:,.0f}", 
                        "start": str(row['Date']), 
                        "color": "#dc3545"
                    })
        
        calendar(events=calendar_events, options={"headerToolbar": {"left": "prev,next today", "center": "title", "right": ""}, "initialView": "dayGridMonth"})
        # --- കലണ്ടർ ലോജിക് അവസാനിക്കുന്നു ---
        
        st.subheader("🗂️ Categorywise Expense Breakdown")
        # ... (ബാക്കി കാറ്റഗറി കോഡ് പഴയതുപോലെ തുടരുക)
