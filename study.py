...
st.markdown("""
    ...
    .stButton>button { 
        ...
        font-weight: bold; width: 100%; border: 2px solid #FFD700;
    }
    </style>
...
with st.form("entry_form", clear_on_submit=True):
    ...
    if st.form_submit_button("SAVE TO SHEET"):
        if item_val and price_val:
            # തീയതി കോഡ് തന്നെ ഉണ്ടാക്കി അയക്കും
            current_date = datetime.now().strftime("%Y-%m-%d")
            
            # നിന്റെ ഫോമിലെ ചോദ്യങ്ങൾക്കുള്ള ID-കൾ (ഇത് വളരെ ശ്രദ്ധിക്കണം)
            payload = {
                "entry.812300063": current_date, # Date
                "entry.685338308": item_val,     # Item
                "entry.464670068": str(price_val) # Amount
            }
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.error("Error: ഡാറ്റ ഫോമിലേക്ക് പോയില്ല!")
            except:
...
try:
    ...
    st.warning("ഡാറ്റ പുതുക്കാൻ 5 സെക്കൻഡ് കാത്തിരിക്കുക...")
