...
def process_voice(text):
    ...
    for word in words:
        ...
        elif word in num_map: amt = int(word); amt = int(num_map[word])
        else: item += word + " "
...
if menu == "🏠 Dashboard":
    ...
    if df is not None and not df.empty:
        # നിങ്ങളുടെ ഷീറ്റിലെ 4-ാമത്തെ കോളം (Debit) തുകയായി എടുക്കുന്നു
...
# --- 💰 Add Entry ---
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        if st.form_submit_button("SAVE TO CLOUD"):
            if item and amt:
                # നിങ്ങൾ അയച്ച ലിങ്കിലെ പുതിയ Entry ID-കൾ ഇവിടെ ചേർത്തു
                payload = {
                ...
                if res.status_code == 200 or res.status_code == 0:
                    st.success(f"{item} - ₹{amt} ഷീറ്റിലേക്ക് സേവ് ചെയ്തു! ✅")
                    st.cache_data.clear()
                else:
                    st.error("സേവ് ചെയ്യാൻ പറ്റിയില്ല. ഇന്റർനെറ്റ് കണക്ഷൻ പരിശോധിക്കൂ.")
...
st.sidebar.write("---")
st.sidebar.write("PAICHI v16.4 | Faisal PRO")
