...
with st.form("my_entry_form", clear_on_submit=True):
    ...
    if st.form_submit_button("SAVE TO SHEET"):
        if item_val and price_val:
            # നീ തന്ന ലിങ്കിൽ നിന്നുള്ള ശരിക്കുള്ള ID-കൾ ഇവിടെ ചേർക്കുന്നു
            payload = {
                "entry.1069832729": datetime.now().strftime("%Y-%m-%d"), # Date
                "entry.1896057694": item_val,                             # Item
                "entry.1570426033": str(price_val)                       # Amount
            }
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.error("ഫോമിലേക്ക് ഡാറ്റ എത്തിയില്ല!")
            except:
...
try:
    ...
    if not df.empty:
        # കോളങ്ങളുടെ പേര് ശരിയാക്കുന്നു (Timestamp, Date, Item, Amount)
        df.columns = [f'Col{i}' for i in range(len(df.columns))]
        # അവസാന കോളമായ Amount നമ്പറാക്കി മാറ്റുന്നു
        ...
        # Item-ഉം Amount-ഉം മാത്രം കാണിക്കുന്നു
        st.table(df.iloc[:, [2, -1]].tail(10).iloc[::-1])
    else:
        st.info("ഷീറ്റിൽ ഡാറ്റയൊന്നുമില്ല. ആദ്യത്തെ എൻട്രി അടിക്കൂ!")
except:
    st.warning("ഡാറ്റ ലോഡ് ചെയ്യുന്നു... 10 സെക്കൻഡ് കാത്തിരിക്കൂ.")
