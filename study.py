...
# 1. സെക്യൂരിറ്റി - പാസ്‌വേഡ് സെറ്റിംഗ്സ്
def check_password():
    ...
    if not st.session_state["password_correct"]:
        ...
        if st.button("LOG IN"):
            if password == "1234": # നിനക്ക് ഇഷ്ടമുള്ള പാസ്‌വേഡ് ഇവിടെ മാറ്റാം
...
# പാസ്‌വേഡ് ശരിയാണെങ്കിൽ മാത്രം ബാക്കി കോഡ് വർക്ക് ചെയ്യും
if check_password():
    ...
    st.set_page_config(page_title="PAICHI Secure Hub", layout="wide")
    ...
    st.markdown("""
        ...
        .total-box { background-color: #000; color: #FFD700; padding: 25px; border-radius: 15px; text-align: center; font-size: 30px; font-weight: bold; border: 4px solid #FFD700; margin-bottom: 20px; }
    ...
    # --- 2. ANALYTICS & WHATSAPP (ഇവിടെ ലിസ്റ്റ് കാണാം) ---
        ...
        if file:
            ...
            if st.button("SCAN"):
                ...
                if reader:
                    ...
                    st.success(f"Detected: {res[0][1] if res else 'No amount found'}")

    st.sidebar.button("Log Out", on_click=lambda: st.session_state.update({"password_correct": False}))
